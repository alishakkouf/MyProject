...
using MyProject.Areas.Admin.BusinessDtos;
using MyProject.Areas.Admin.Dtos;
...
namespace MyProject.Areas.Admin
{
           ...
           IBusinessManager businessManager, IMapper Mapper) : BaseAdminController(factory)
    {
        ...
        /// </summary>
        [HttpGet("GetAll")]
        public async Task<ActionResult<List<BusinessDto>>> GetAllAsync(GetAllBusinessQuery query)
        {
            _logger.LogInformation($"Getting list of admins(owner of businesses) via {nameof(GetAllAsync)} API");

            var data = await _businessManager.GetAllAsync(query);
